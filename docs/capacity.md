...
| | NORMAL | PERFORMANCE |
|---|---|---|
| DynamoDB billing | **Provisioned 5/5 per table — $0** (free tier) | **On-demand — pay-per-request** (~$0.625 per million writes, $0.125 per million reads) |
| Item size (both modes) | ≤ 400 KB (DynamoDB's hard limit) | same |
| Per-app write budget | 800 **write-units/min per app** (~13/s — half the free pool, burst-credit for spikes) | Guardrail only: 100 000 units/min |
| Per-app read budget | 800/min (~16 KB/s) | Guardrail only: 400 000/min |
| Total per app | 2 000/min | 500 000/min |
| Platform pool | 2 400/min (the true combined free-tier pool) | 2 000 000/min |
