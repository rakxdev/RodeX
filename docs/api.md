...
Response shape: `{ "ok": true, "result": {...} }` | `{ "ok": false, "error": { "code", "message" } }`
...
```
`strong: true` = strongly consistent read (costs 2×). 404 if missing.
...
| `DELETE /v1/admin/apps/:id` | soft delete (5-min recovery window) |
