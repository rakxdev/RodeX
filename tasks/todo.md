...
### Phase A: Gateway foundation (key CRUD + table delete)
- [ ] A1 rodex_mcp_keys table + storage helpers (hash-only + 48h cipher, create/list/view/delete)
- [ ] A2 Admin endpoints: POST/GET /v1/admin/mcp/keys · POST :id/view · DELETE :id (no rotation)
- [ ] A3 POST /v1/table/delete (app auth, owned-only, 403 cross-app)
### Checkpoint: gateway green → deploy
### Phase B: MCP worker skeleton + read-only tools
- [ ] B1 mcp/ worker: /mcp Streamable HTTP, Bearer auth vs key table, Origin validation, MCP budgets
- [ ] B2 read-only tools: health, get_instructions, list_apps, get_app, list_tables, get_item, query
### Checkpoint: read-only live (MCP Inspector) → human review
### Phase C: Mutations + confirmation gate
- [ ] C1 gate: mutating tools refuse without confirmed:true (structured refusal)
- [ ] C2 mutation tools: put/update/delete_item, create/delete_table, create/delete_app
### Checkpoint: full data-plane live with confirmations → human review
### Phase D: Dashboard MCP page
- [ ] D1 /mcp route + header button + key management (create/reveal 48h/list/view/delete)
- [ ] D2 operating manual panel (client recipes, tool table, confirmation protocol, budgets, FAQ)
### Checkpoint: page reviewed by human
### Phase E: Docs, CI, deploy, verify
- [ ] E1 docs: api.md/openapi.yaml sync, docs/mcp.md, ADR-006, CHANGELOG, usage page section
- [ ] E2 CI covers mcp/, deploy ships gateway + mcp + dashboard; live demo with user
