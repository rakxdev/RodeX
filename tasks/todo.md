...
- [ ] T1 Scaffold repo: package.json, tsconfig (strict), vitest, wrangler.toml
...
- [ ] T2 Core libs: `limits.ts`, `auth.ts` (HMAC-SHA256, salt, constant-time),
...
- [ ] T3 Storage interface + mock adapter + aws adapter (aws4fetch, SigV4,
...
- [ ] T4 Registry + soft-delete state machine: create/list/get/rotate/revoke,
...
- [ ] T5 Items + tables API: put/get/update/delete/query, `app_<id>_` prefix,
...
- [ ] T6 Admin auth: GitHub OAuth (start/callback, state, allowed users) +
...
- [ ] T7 Rate limiting: [[ratelimits]] bindings + `rate.ts` envelope
...
- [ ] T8 Dashboard (Pages): login (GitHub + password), apps list/create,
...
- [ ] T9 Docs: api.md (endpoints, errors, retry helper), testing.md
...
- [ ] T10 Final: all SPEC §11 criteria, README (how to run/deploy), commit,
